...
alter table crm_sys_sales_target_norm_type drop column reclanguage;
alter table crm_sys_sales_target_norm_type add name_lang jsonb;
drop function "public"."crm_func_sales_target_norm_type_save"("_normtypeid" uuid, "_normtypename" text, "_entityid" uuid, "_fieldname" text, "_calcutetype" int4, "_userno" int4);
drop FUNCTION "public"."crm_func_sales_target_norm_type_save"("_normtypeid" uuid, "_normtypename" text, "_entityid" uuid, "_fieldname" text, "_calcutetype" int4, "_userno" int4, "_reclanguage" jsonb);
...
BEGIN
	...
	*/
	...
	SELECT normtypeid,normtypename,recorder,isdefault,calcutetype,name_lang FROM  crm_sys_sales_target_norm_type 
	WHERE recstatus=1
	 ...
	 BEGIN
	    ...
	    IF _normtypename IS NOT  NULL AND  _normtypename!='' THEN
	      ...
	      IF _normtypeid IS NULL OR _normtypeid='00000000-0000-0000-0000-000000000000'::uuid THEN  --新增一条数据  
			...
			--插入数据
			INSERT INTO crm_sys_sales_target_norm_type (normtypename,entityid,fieldname,calcutetype,recorder,reccreator,recupdator,name_lang)
