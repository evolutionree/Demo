...
{
    public static class TaskJobHelper
    {
        ...
        static TaskJobHelper()
        {
            IConfigurationRoot config = ServiceLocator.Current.GetInstance<IConfigurationRoot>();
            ScheduleSetting _config = config.GetSection("ScheduleSetting").Get<ScheduleSetting>();

            //读取作业调度服务的配置
            ScheduleServerHost = _config.TaskJobServer;
        ...
        public static TaskJobModel CreateCustomReminder(string jobName, string eventTitle, string enterpriseNo, string eventId)
        {
            var jobNameArr = JobNameSplitor(jobName);

            var model = new TaskJobModel();
            model.Assembly = jobNameArr[1].Equals("{Default}") ? DefaultAssemblyName : jobNameArr[1]; ;
            model.Type = jobNameArr[0];
            model.ClientId = enterpriseNo;
            model.CronStrings = new List<string>() { "0 0 4 * * ?" };  //

            // model.CronStrings = new List<string>() { "0 0/1 * * * ?" };

            model.JobData = new Dictionary<string, object>();
            model.JobData.Add("eventId", eventId);
            model.MessageId = eventId;
            model.UserId = 0;
            model.ReMark = "自定义事件提醒:" + eventTitle;
            return model;
        }
