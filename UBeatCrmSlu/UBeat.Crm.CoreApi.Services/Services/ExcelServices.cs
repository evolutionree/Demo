...
namespace UBeat.Crm.CoreApi.Services.Services
{
	public class ExcelServices : BasicBaseServices
	{
     ...
     //               NotifyEntity notifyEntity = new NotifyEntity();
		...
		public ExportModel ExportData(ExportDataModel data)
		{
			...
			foreach (var m in sheetDefine)
			{
				...
				else //动态实体配置方式导出
				{
					...
					var tempFields = template.Headers.Where(o => o.FieldType == FieldType.Image || o.FieldType == FieldType.Address || o.FieldType == FieldType.reference);
			...
			if (sheetTemplate.DataObject is List<DynamicEntityDataFieldMapper>)
			{
				...
				foreach (var m in typeFields)
				{
					...
					else if (data != null)
					{
						...
						result = CheckFieldData(m, columnValue, rowdata, userno, out errorMsg);
					}
		...
		//校验字段的数据格式和处理特殊控件
		private bool CheckFieldData(DynamicEntityDataFieldMapper typeField, string columnValue, Dictionary<string, object> rowdata, int userno, out string errorMsg)
		{
			errorMsg = null;
			switch ((DynamicProtocolControlType)typeField.ControlType)
			{
