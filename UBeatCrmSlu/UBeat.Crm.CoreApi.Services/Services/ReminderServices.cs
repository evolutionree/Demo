...
namespace UBeat.Crm.CoreApi.Services.Services
{
    public class ReminderServices : BaseServices
    {
        ...
        private readonly static string lockObject = "qrtSyncCustomer";
            ...
            if (dataList != null && dataList.Count > 0)
            {
                ...
                foreach (var item in ListDic)
                {
                    ...
                    if (dataItem != null)
                    {
                        ...
                        if (reminder.RoleId.HasValue)
                        {
                            var roleRepository = ServiceLocator.Current.GetInstance<IRoleRepository>();
                            var roleUsers = roleRepository.RoleUserQuery(new DomainModel.Role.RoleUserMapper
                            {
                                DeptId = "7f74192d-b937-403f-ac2a-8be34714278b",
                                PageIndex = 1,
                                PageSize = int.MaxValue,
                                RoleId = reminder.RoleId.Value.ToString(),
                                UserName = string.Empty
                            }, userNumber);
                            var users = roleUsers["Page"];
                            List<int> roleRec = new List<int>();
                            foreach (var u in users)
                            {
                                roleRec.Add(Convert.ToInt32(u["userid"].ToString()));
                            }
                            if (!receiveDic.ContainsKey(MessageUserType.SpecificUser) && roleRec.Count > 0)
                                receiveDic.Add(MessageUserType.SpecificUser, roleRec);
                            else
                                receiveDic[MessageUserType.SpecificUser] = receiveDic[MessageUserType.SpecificUser].Concat(roleRec).Distinct().ToList();
                        }
        ...
        private void QrtReminderMsg(int userNumber, ReminderMapper reminder)
        {
            var dataList = _repository.CallFunction(reminder.FunctionName, reminder.RecId.ToString(), userNumber);
            if (dataList != null && dataList.Count > 0)
            {
                ...
                for (int i = 0; i < dataList.Count; i++)
                {
                    ...
                    var recId = string.Concat(dataList[i]["recid"]);
                    if (reminder.BusRecId.HasValue && recId != reminder.BusRecId.Value.ToString()) continue;
                    if (!ListDic.ContainsKey(recId))
                        ListDic.Add(recId, new List<IDictionary<string, object>>() { dataList[i] });
                    else
                        ListDic[recId].Add(dataList[i]);
                ...
                foreach (var item in ListDic)
                {
                    ...
                    if (dataItem != null)
                    {
                        ...
                        if (receiveDic.Count > 0)
                        {
                            var templateKeyValue = new Dictionary<string, string>();
                            templateKeyValue.Add("title", string.Concat(dataItem["title"]));
                            templateKeyValue.Add("content", string.Concat(dataItem["content"]) + "," + reminder.Message);
                            templateKeyValue.Add("pushcontent", string.Concat(dataItem["pushcontent"]));
        ...
        public OutputResult<object> QrtExecuteTaskNow(Dictionary<string, object> dic)
        {
            lock (lockObject)
            {
                bool isSuccess = false;

                ReminderMapper _reminder = _repository.GetReminderById(Guid.Parse(dic["reminderid"].ToString()), 1);
                if (dic.ContainsKey("busrecid"))
                {
                    _reminder.BusRecId = Guid.Parse(dic["busrecid"].ToString());
                    _reminder.Message = dic["message"].ToString();
                }
                if (_reminder != null)
                {
                    if (_reminder.RecStatus == 0)
                    ...
                    if (isNeedSchedule == ScheduleTypeEnum.ScheduleTypeNeed)
                    {
                        var model = ScheduleServices.Creator(ScheduleServices.CustomerTipsJobName, _reminder.RecVersion.ToString(), dic["reminderid"].ToString(), 1.ToString());
                        var result = ScheduleServices.ExecuteNow(model);
