...
namespace UBeat.Crm.CoreApi.Services.Services
{
    public class DbManageServices : EntityBaseServices
    {
        ...
        private IDbWorkFlowRepository _dbWorkFlowRepository;
        public DbManageServices(IDbManageRepository dbManageRepository, IDbWorkFlowRepository dbWorkFlowRepository)
        {
            this._dbManageRepository = dbManageRepository;
            _dbWorkFlowRepository = dbWorkFlowRepository;
        }
        /// <summary>
        /// 创建函数、存储过程的脚本
        /// </summary>
...
WITH (OIDS=FALSE);", tablename, fieldSQL);
        ...
        public OutputResult<object> SaveWorkFlowInfoList(string jsontext, int userNum)
        {
            var flowInfos = JsonConvert.DeserializeObject<List<DbWorkFlowInfo>>(jsontext);
            _dbWorkFlowRepository.SaveWorkFlowInfoList(flowInfos, userNum, null);
