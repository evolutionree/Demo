...
namespace UBeat.Crm.CoreApi.Core.Utility
{
    ...
    {
        //定义拼音区编码数组
        private static readonly int[] PinYinCodes = {
        ...
        //定义拼音数组
        private static readonly string[] PinYinNames =
        {
        ...
        public static string ToChinese(string chstr,bool isShort)
        {
            if (string.IsNullOrWhiteSpace(chstr)) return string.Empty;
            ...
            foreach (char t in mChar)
            {
                ...
                {
                    var arr = System.Text.Encoding.GetEncoding("GB2312").GetBytes(t.ToString());
                    int m1 = arr[0];
                    int m2 = arr[1];
                    ...
                    else
                    {
                        switch (asc)
                        {
                            ...
                            default:
                                ...
                                {
                                    if (PinYinCodes[i] <= asc) //判断汉字的拼音区编码是否在指定范围内
                                    {
                                        if (isShort)
                                        {
                                            pystr += PinYinNames[i].Substring(0,1); //如果不超出范围则获取对应的拼音
                                        }
                                        else
                                        {
